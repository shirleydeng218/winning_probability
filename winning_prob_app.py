    ...
    for objective in metrics_df['conversion_segment'].unique():
        ...
        for cell in metrics_df[(metrics_df['conversion_segment']==objective)]['study_name'].unique():
            ...
            for dt in metrics_df['analysis_date'].unique():
                ...
                pdiff = (test_rate - ctl_rate) * sub_df_sub['treatment_user_count'][0]
    ...
    for dt in results['dt'].unique():
        ...
        for metric in results['metric'].unique():
            ...
            for i in range(len(sub_results['cell'].unique())):
                ...
                # added in to calculate incremental lift samples for each cell
...
Results are generated using Hulu’s proprietary Winning Probability framework, which incorporates statistical uncertainty to provide a more reliable view of relative performance in addition to traditional significance testing.
